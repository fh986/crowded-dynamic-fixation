...
for subj = 1:numSubj 
    ...
    for block = 1:length(all_conditions)
        ...
        for trial = this_trials_idx'
            ...
            if size(currenttrial_ee,1) > 0
                trial_tracking_error_Px(trial) = calcTrackErrorForTrial(currenttrial_ee);
            else
                trial_tracking_error_Px(trial) = -1000;
            end
    ...
    % convert to deg
    trial_tracking_error_Deg = (trial_tracking_error_Px .* px_to_deg)';
...
function [trialError] = calcTrackErrorForTrial(currenttrial_ee)
